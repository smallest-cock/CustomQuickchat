...
bool WebsocketClientManager::connect(const std::string &uri, JsonMsgHandler msgHandler) {
	...
	con->set_open_handler([this](connection_hdl hdl) {
		LOG("Connected to websocket server");
		m_isConnected.store(true);
		m_isConnecting.store(false);
	});
	con->set_fail_handler([this](connection_hdl hdl) {
	 ...
	 */
	con->set_close_handler([this](connection_hdl hdl) {
		auto connection = m_endpoint.get_con_from_hdl(hdl);
	...
	con->set_message_handler([this](connection_hdl hdl, WebsocketClient::message_ptr msg) {
		m_isConnected.store(true);
		m_isConnecting.store(false);
