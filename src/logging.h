...
template <typename... Args> void LOG(std::wstring_view format_str, Args&&... args)
{
...
template <typename... Args> void LOGERROR(std::wstring_view format_str, Args&&... args)
{
	_globalCvarManager->log("ERROR: " + std::vformat(format_str, std::make_wformat_args(args...)));
}

template <typename... Args> void DEBUGLOG(const FormatString& format_str, Args&&... args)
{
...
template <typename... Args> void DEBUGLOG(const FormatWstring& format_str, Args&&... args)
{
	if constexpr (DEBUG_LOG)
	{
